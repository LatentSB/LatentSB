...
class StableDiffusion():
    def __init__(self,
        ...
        total_sigmas = (1-self.scheduler.alphas_cumprod).sqrt() / self.scheduler.alphas_cumprod.sqrt()
        self.log_sigmas = total_sigmas.log().to(device)
    ...
    def sigma_to_t(self, sigma: torch.FloatTensor):
        log_sigma = sigma.log()
        dists = log_sigma.to(self.device) - self.log_sigmas[None, :]
        return dists.abs().argmin(dim=-1).view(sigma.shape)

    def alpha_to_t(self, alpha_s, min_s:float=0.0, max_s:float=1000.0):
        all_alphas = self.scheduler.alphas_cumprod
        dists = all_alphas - alpha_s
        low_idx = dists.ge(0).cumsum(dim=0).argmax(dim=0).clamp(max=len(all_alphas)-2)
        high_idx = low_idx + 1
        low, high = all_alphas[low_idx], all_alphas[high_idx]
        w = (low-alpha_s) / (low-high)
        w = w.clamp(0, 1)
